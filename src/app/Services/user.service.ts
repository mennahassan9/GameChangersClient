...
export class UserService {
  ...
  getUsersC(chapter) {
    ...
    const currentToken = this.localStorageService.get('token');
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    return this.http.get(environment.apiUrl +`/users/chapter/${chapter}`, { headers: reqHeaders })
    .map(res => res.json());
  }
  getUsersR(region) {
    const reqHeaders: Headers = new Headers();
    ...
    const currentToken = this.localStorageService.get('token');
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    return this.http.get(environment.apiUrl +`/users/region/${region}`, { headers: reqHeaders })
    .map(res => res.json());
  }
  ...
  sendToC(chapter,email){
    ...
    let body= JSON.stringify(email);
    return this.http.post(environment.apiUrl +"/users/email/chapter",{chapter, subject: email.subject, emailBody: email.body }, { headers: reqHeaders })
              .toPromise();
  }
  sendToR(region, email){
    ...
    let body= JSON.stringify(email);
    return this.http.post(environment.apiUrl +"/users/email/region",{region, subject: email.subject, emailBody: email.body }, { headers: reqHeaders })
              .toPromise();
  }
  ...
  sendEmails(mailList, email): Observable<any> {
    ...
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    reqHeaders.append('Content-Type', 'application/json');
    return this.http.post(environment.apiUrl + `/users/email/global`, {emails: mailList, subject: email.subject, emailBody: email.body}, { headers: reqHeaders });
  }
  getAllUsers() {
    const reqHeaders: Headers = new Headers();
    reqHeaders.append('Content-Type', 'application/json');
    const currentToken = this.localStorageService.get('token');
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    return this.http.get(environment.apiUrl + `/users/allusers`, { headers: reqHeaders })
    .map(res => res.json());
  }

...
export class TeamService {
  ...
  getTeamsC(chapter) {
    ...
    const currentToken = this.localStorageService.get('token');
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    return this.http.get(environment.apiUrl +`/teams/chapter/${chapter}`,{ headers: reqHeaders })
    .map(res => res.json());
  }
  getTeamsR(region) {
    const reqHeaders: Headers = new Headers();
    ...
    const currentToken = this.localStorageService.get('token');
    reqHeaders.append('Authorization', 'Bearer ' + currentToken);
    return this.http.get(environment.apiUrl +`/teams/region/${region}`,{ headers: reqHeaders })
    .map(res => res.json());
  }
